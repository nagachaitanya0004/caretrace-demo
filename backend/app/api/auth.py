...
async def signup(payload: UserCreate):
    ...
    try:
        ...
        logger.error("PostgreSQL user creation error: %s — %r", type(exc).__name__, exc)
    ...
    try:
        ...
        await pg_session.rollback()
        await pg_session.close()
        logger.error("MongoDB insert error: %s — %r", type(exc).__name__, exc)
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
    ...
    try:
        ...
        logger.error("PostgreSQL commit error: %s — %r — compensating MongoDB delete", type(exc).__name__, exc)
        ...
        for attempt in range(1, 4):
            try:
                ...
                logger.error("Mongo cleanup error on attempt %d: %s — %r", attempt, type(cleanup_exc).__name__, cleanup_exc)
